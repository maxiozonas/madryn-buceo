    ...
    <header className="sticky top-0 z-50 shadow-md text-white bg-negro pt-2">
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
        ...
        <div className="hidden lg:flex items-center gap-4">
          <Button size="lg" className="bg-rojo hover:bg-rojo/50 hover:shadow-lg text-white transition-all duration-300 text-lg font-semibold">
      ...
      >
        <div className="container h-full flex flex-col py-6 px-4">
          <nav className="flex flex-col flex-1 space-y-2">
            {NavLinks.map((link, index) => (
              <div key={index} className="rounded-lg bg-negro-secundario/50 shadow-sm overflow-hidden">
                {link.submenu ? (
                  <>
                    <button
                      ...
                      <div className="px-6 py-2 bg-[#1f1d1e]">
                        {link.submenu.map((sublink, subIndex) => (
                          <Link
                            key={subIndex}
                            href={sublink.href}
                            className="block py-3 text-gray-200 hover:text-rojo transition-colors duration-200"
          ...
          <div className="mt-6">
            <Button
              className="w-full bg-rojo hover:bg-rojo/50 hover:shadow-lg text-white transition-all duration-300 text-lg font-semibold"
              onClick={() => setIsOpen(false)}
            >
              <Link href="/reservar" className="w-full">
                Reservar Ahora
